...
class UploadImg {	
  ...
  // Set to true if whitespace in a filename is replaced by underscores
  ...
  public function __construct($path) {
    ...
    // If a valid directory or writeable, the path that is passed is assigned to the _destination property, and the actual file
  ...
  public function getMaxSize() {
	return number_format($this->_max/1048576, 1) . "MB";
  }
  ...
  // $overwrite added as argument; false sets the default value and makes the argument optional; if you want to overwrite files, 
  ...
  public function move($overwrite = false) {
	...
	$field = current($this->_uploaded); // $_uploaded contains $FILES array (the actual file array)
    // Check if $field["name"] is an array, if it is, then the foreach loop is created
	if (is_array($field["name"])) {
	  ...
	  foreach ($field["name"] as $number => $filename) {
		...
		$this->_renamed = false;
        // At each iteration, values extracted from the current element of $_FILES are then passed to the processFiles() method 
        // (remember $field[] is $_FILE[]), e.g. via the current element's key ($number), its error, size, type, and location 
        // are accessed
  ...
  // When called, this returns any message passed to the &_messages property of the class
  public function getMessages() {
	return $this->_messages;
  }
  
  // Check for errors and pass any to the _messages property
  // Passed the filename (name from $field/$_FILE) and the error level from $field["error"]
  protected function checkError($filename, $error) {
	switch ($error) {
	  case 0: // checks that $_FILES["image"]["error"] == 0 (no errors)
		return true;
	  case 1:
	  case 2:
  ...
  protected function checkSize($filename, $size) {
	...
	if ($size == 0) {
	  return false; // false because no file to save and the error message will have been created by checkError()
  ...
  // Get the filename(s); simply returns the value of the _filenames property
  ...
  // Passed a filename and a boolean variable that determines whether to overwrite the existing file
  protected function checkName($name, $overwrite) {
    ...
    // Set whether to rename the file if one with the same name already exists or overwrite it
	...
	if (!$overwrite) {
	  ...
	  if (in_array($nospaces, $existing)) {
		...
		if ($dot) {
          ...
          // Count the characters starting from $dot to end of the string (the file extension name)
		  $extension = substr($nospaces, $dot);
		} else {
          // If $dot is false (e.g. name doesn't have .jpg), the full name is stored in $base and $extension is empty
		  $base = $nospaces;
		  $extension = "";
		}
		...
		$this->_renamed = true; // and since it is renamed, _renamed property is set to true
	  }
	}
	return $nospaces; // the final filename result is returned
  }

  // Method that processes the file, method is called with parameter values inside the move() method
  protected function processFile($filename, $error, $size, $type, $tmp_name, $overwrite) {
	$OK = $this->checkError($filename, $error); // checks filename for error level, the resulting true or false is saved to $OK
    ...
    if ($OK) {
	  ...
	  if ($sizeOK && $typeOK) {
		...
		// If successfully uploaded, save the filename to the $_filenames array and display message
        if ($success) {
	      ...
	      $this->_filenames[] = $name;
			...
			if ($this->_renamed) {
			  $message .= " and renamed {$name}"; // a string is added to the successful upload message string stored in $message
			}
			$this->_messages[] = $message; // the complete message is then assigned to the $_messages array
		} else {
          // Otherwise the file was not successfully uploaded    
