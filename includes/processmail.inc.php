...
if (!$suspect) {
  foreach ($_POST as $key => $value) {
	// Assign name attribute to a variable $key and the value to a variable $value.
	...
	if (empty($temp) && in_array($key, $required)) {
	  ...
	  ${$key} = $temp; // this should have $name, $email, and $comment, etc, containing respective values
	}
  }
}
...
// It's set to false until $mail() has succeeded; below is the code that builds the message.
$mailSent = false;
...
if (!$suspect && !$missing && !$issues) {
  ...
  // When called, mail() returns true if it succeeds in handing the email to the web server's mail transport agent.
  if (!$mailSent) { // if it fails
